    ...
    <div className={`fixed inset-0 z-[200] flex flex-col transition-all duration-1000 overflow-hidden ${mood === 'happy' ? 'bg-[#fffbeb]' : 'bg-[#020617]'}`}>
      <header className={`relative z-10 p-8 flex items-center justify-between transition-colors border-b ${mood === 'happy' ? 'bg-white/40 border-amber-100' : 'bg-slate-900/40 border-white/5'} backdrop-blur-xl`}>
        <div className="flex items-center gap-6">
          <button onClick={onBack} className={`p-4 rounded-2xl transition-all shadow-sm ${mood === 'happy' ? 'bg-white text-amber-600' : 'bg-white/5 text-slate-400'}`}>
            <X size={24} />
          </button>
          <div>
        ...
        <button onClick={onComplete} className={`px-10 py-4 rounded-2xl font-black text-sm text-white shadow-xl transition-all active:scale-95 ${mood === 'happy' ? 'bg-amber-500 shadow-amber-500/20' : 'bg-blue-600 shadow-blue-500/20'}`}>
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-8 relative z-10 gap-20">
        <div className="flex items-center justify-center gap-12 lg:gap-24 w-full">
           <div className="flex flex-col items-center gap-4">
             <button onClick={() => handleMoodSwitch('happy')} className={`group w-28 h-28 rounded-[2.5rem] flex flex-col items-center justify-center transition-all duration-500 ${mood === 'happy' ? 'bg-amber-400 shadow-[0_20px_50px_rgba(251,191,36,0.5)] scale-110 border-4 border-white' : 'bg-white/10 opacity-30 grayscale hover:opacity-100 hover:grayscale-0'}`}>
           ...
           <div className="relative">
              <div className={`relative w-56 h-56 rounded-[4.5rem] flex flex-col items-center justify-center text-9xl shadow-2xl transition-all duration-700 ${mood === 'happy' ? 'bg-white scale-105 rotate-2 animate-bounce-subtle' : 'bg-slate-800 -rotate-2'}`}>
                🐰
              </div>
              <div className={`absolute -top-4 -right-4 px-6 py-2 rounded-full border-2 font-black text-xs shadow-xl transition-all duration-700 ${mood === 'happy' ? 'bg-amber-100 border-amber-400 text-amber-600 scale-100' : 'bg-blue-900 border-blue-400 text-blue-300 scale-90'}`}>
                 {mood === 'happy' ? '☀️ 快乐大调' : '🌙 忧郁小调'}
              </div>
           </div>

           <div className="flex flex-col items-center gap-4">
             <button onClick={() => handleMoodSwitch('sad')} className={`group w-28 h-28 rounded-[2.5rem] flex flex-col items-center justify-center transition-all duration-500 ${mood === 'sad' ? 'bg-blue-600 shadow-[0_20px_50px_rgba(37,99,235,0.5)] scale-110 border-4 border-white/20' : 'bg-white/10 opacity-30 grayscale hover:opacity-100 hover:grayscale-0'}`}>
        ...
        <div className={`w-full max-w-5xl p-12 rounded-[4.5rem] border transition-all duration-1000 shadow-2xl ${mood === 'happy' ? 'bg-white border-amber-100' : 'bg-slate-900/80 border-white/5'}`}>
           <div className="flex flex-col gap-10">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-5">
                   <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-white shadow-lg ${mood === 'happy' ? 'bg-amber-500' : 'bg-blue-600'}`}>
                ...
                <button 
                  onClick={togglePlay}
                  className={`px-10 py-4 rounded-full font-black text-lg text-white transition-all shadow-xl flex items-center gap-4 ${isPlaying ? 'bg-rose-500 shadow-rose-500/20' : mood === 'happy' ? 'bg-amber-500 shadow-amber-500/20' : 'bg-blue-600 shadow-blue-500/20'} active:scale-90`}
                >
                ...
                {[...Array(8)].map((_, idx) => {
                  ...
                  return (
                    <div key={idx} className="flex flex-col items-center gap-4">
                      <div className={`w-full aspect-square rounded-[2.2rem] transition-all duration-500 flex items-center justify-center border-4 ${hasNote ? mood === 'happy' ? 'bg-amber-400 border-amber-200 shadow-[0_10px_30px_rgba(251,191,36,0.3)]' : 'bg-blue-600 border-blue-400 shadow-[0_10px_30px_rgba(37,99,235,0.3)]' : isDark ? 'bg-white/5 border-white/5' : 'bg-slate-50 border-slate-100'} ${isActive ? 'scale-110 ring-8 ring-current/10' : 'opacity-80 scale-100'}`}>
