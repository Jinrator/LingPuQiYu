...
import { ArrowLeft, Globe, Type, Check, UserCog, ArrowRight } from 'lucide-react';
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-8 py-6 sm:py-12 space-y-5 sm:space-y-8 pb-20 md:pb-12">
        ...
        >
          <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: PALETTE.blue.bg }}>
            <UserCog size={18} style={{ color: PALETTE.blue.accent }} />
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-bold text-slate-800">{t('account.goToAccount')}</p>
            <p className="text-xs font-medium text-slate-400 mt-0.5">{t('account.goToAccountDesc')}</p>
          </div>
